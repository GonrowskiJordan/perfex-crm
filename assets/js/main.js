...
$("body").on("loaded.bs.select change", "select.ajax-search", function(e) {
  ...
  if ($elmWrapper.find(".ajax-clear-values").length === 0) {
    ...
    dropdownToggle.after('<span class="pointer ajax-clear-values" onclick="deselect_ajax_search(this); return false;" data-id="' + id + '"></span>');
  }
});
...
$(document).on("mousemove", function(e) {
  if (!is_mobile() && $("body").hasClass("hide-sidebar")) {
    if (
      (isRTL == "true" && $(window).width() - e.pageX <= 10) || (isRTL == "false" && e.pageX <= 10)) {
      $(".hide-menu").click();
    }
  }
});
$(function() {
  ...
  $(document).on("app.form-validate", function(e, form) {
    ...
    $(form).on("change", "select.ajax-search, select.selectpicker", function(e) {
      if ($(this).selectpicker("val") && !$(this).is(":disabled")) {
        if (typeof $(this).rules() != "undefined" && Object.keys($(this).rules()).length === 1 && $(this).rules().hasOwnProperty("required")) {
  ...
  $("#search_input").on("keyup paste" + (app.browser == "safari" ? " input" : ""), function() {
    ...
    if (q.length < 2 && app.user_language.indexOf("chinese") === -1 && app.user_language.indexOf("japanese") === -1) {
      return;
    }
  ...
  $("body").on("change", "#task-modal #checklist_items_templates", function() {
    ...
    if (valTemplate !== "") {
      var $taskModal = $("#task-modal");
      add_task_checklist_item($taskModal.find("[data-task-single-id]").attr("data-task-single-id"), valTemplate);
  ...
  $("#task-modal").on("change", 'input[name="timesheet_start_time"], input[name="timesheet_end_time"], input[name="timesheet_duration"]', function() {
    if ($(this).val() && $(this).parents(".form-group").hasClass("has-error")) {
  ...
  $("body").on("click", ".task-single-add-timesheet", function(e) {
    e.preventDefault();
    var start_time_input = $("body").find('#task-modal input[name="timesheet_start_time"]');
    var end_time_input = $("body").find('#task-modal input[name="timesheet_end_time"]');
    var duration_input = $("body").find('#task-modal input[name="timesheet_duration"]');
    var start_time = start_time_input.val();
    var end_time = end_time_input.val();
    var duration = duration_input.val();
    start_time === "" && start_time_input.parents(".form-group").addClass("has-error");
    end_time === "" && end_time_input.parents(".form-group").addClass("has-error");
    duration === "" && duration_input.parents(".form-group").addClass("has-error");
  ...
  $("body").on("hide.bs.modal", "#task-modal", function(e) {
    tinymce.get().forEach(function(editor) {
      if (editor.id.includes("task_comment") && editor.getContent() !== "" && $(editor.editorContainer).is(":visible")) {
        e.preventDefault();
      }
    });
  });
  ...
  if (calendar_selector.length > 0) {
    ...
    var calendar_settings = {
      ...
      dateClick: function(info) {
        ...
        var d1 = fmt.formatDate(new Date(info.dateStr),
          (vformat = app.options.time_format == 24 ? app.options.date_format + " H:i" : app.options.date_format + " g:i A"));
  ...
  $("body").on("click", '#lead-modal a[data-toggle="tab"]', function() {
    if (this.hash == "#tab_lead_profile" || this.hash == "#attachments" || this.hash == "#lead_notes" || this.hash == "#gdpr" || this.hash == "#lead_activity") {
  ...
  if (table_leads.length) {
    ...
    _table_api = initDataTable(table_leads, admin_url + "leads/table", leadsTableNotSearchable, leadsTableNotSortable, LeadsServerParams,
    ...
    $.each(LeadsServerParams, function(i, obj) {
      $("select" + obj).on("change", function() {
        $("[name='view_status[]']").prop("disabled", $(this).val() == "lost" || $(this).val() == "junk").selectpicker("refresh");
  ...
  $("body").on("change loaded.bs.select", "#item_select", function() {
    ...
    $(".items-wrapper .bootstrap-select").css("max-width", selectWrapper.width() + "px");
  ...
  $("body").on("click", ".cpicker", function() {
    ...
    if ($(this).hasClass("kanban-cpicker")) {
      ...
      $(this).parents(".panel-heading").css("border", "1px solid " + color);
  ...
  if (table_activity_log.length) {
    ...
    initDataTable(table_activity_log, window.location.href, "undefined", "undefined", ActivityLogServerParams,
      [1, "desc"]);
  }
  ...
  if (table_tasks.length) {
    ...
    _table_api = initDataTable(table_tasks, tasksTableURL, tasksTableNotSortable, tasksTableNotSortable, TasksServerParams,
      [table_tasks.find("th.duedate").index(), "asc"]);
    if (_table_api && $("body").hasClass("dashboard")) {
      _table_api.column(5).visible(false, false).column(6).visible(false, false).columns.adjust();
    }
  }
  ...
  $("body").on("submit", "._transaction_form", function() {
    ...
    if ($previewItem.find('[name="description"]').length && $previewItem.find('[name="description"]').val().trim().length > 0 && $previewItem.find('[name="rate"]').val().trim().length > 0) {
  ...
  if ($("#sales-upload").length > 0) {
    new Dropzone("#sales-upload", appCreateDropzoneOptions({
      ...
      success: function(files, response) {
        ...
        if (type == "estimate") {
          $("body").hasClass("estimates-pipeline") ? estimate_pipeline_open(response.rel_id) : init_estimate(response.rel_id);
        } else if (type == "proposal") {
          $("body").hasClass("proposals-pipeline") ? proposal_pipeline_open(response.rel_id) : init_proposal(response.rel_id);
        } else {
        ...
        if (response.success === true || response.success == "true") {
          ...
          data += '<div class="pull-left"><i class="attachment-icon-preview fa-regular fa-file"></i></div>';
  ...
  if (billable_tasks_area.length > 0) {
    ...
    $.each(tasks, function(i, obj) {
      ...
      if (obj.started_timers === true) {
        option_data += 'disabled class="text-danger" data-subtext="' + app.lang.invoice_task_billable_timers_found + '"';
    ...
    tasks_help_wrapper.html('<span class="pointer popover-invoker" data-container=".form-group-select-task_select" data-trigger="click" data-placement="top" data-toggle="popover" data-content="' + help_tooltip + '"><i class="fa-regular fa-circle-question"></i></span>');
    delay(function() {
      if (
        (tasks_help_wrapper.attr("info-shown-count") < 3 || typeof tasks_help_wrapper.attr("info-shown-count") == "undefined") && $(".projects-wrapper").is(":visible") && tasks.length > 0) {
...
function init_rel_tasks_table(rel_id, rel_type, selector) {
  ...
  initDataTable($selector, url, tasksRelationTableNotSortable, tasksRelationTableNotSortable, TasksServerParams,
...
function edit_note(id) {
  ...
  if (description !== "") {
    $.post(admin_url + "misc/edit_note/" + id, {
      ...
      if (response.success === true || response.success == "true") {
        alert_float("success", response.message);
        $("body").find('[data-note-description="' + id + '"]').html(response.description);
      }
    });
    toggle_edit_note(id);
  }
...
function kanban_load_more(status_id, e, url, column_px, container_px) {
  ...
  if (page <= total_pages) {
    ...
    requestGet(buildUrl(admin_url + url, LoadMoreParameters)).done(function(response) {
      ...
      $('[data-col-status-id="' + status_id + '"]').attr("data-total", parseInt($('[data-col-status-id="' + status_id + '"]').attr("data-total")) + $("<ul></ul>").append(response).find("li").length);
...
function print_lead_information() {
  ...
  $leadViewWrapper.find("p").css("font-size", "100%").css("font", "inherit").css("vertical-align", "baseline").css("margin", "0px");
  ...
  mywindow.document.write(".lead-information-col { " + "float: left; width: 33.33333333%;" + "}" + "");
  mywindow.document.write("</style>");
...
function leads_bulk_action(event) {
  if (confirm_delete()) {
    ...
    if (mass_delete == false || typeof mass_delete == "undefined") {
      ...
      if (data.status === "" && data.lost === false && data.assigned === "" && data.source === "" && data.last_contact === "" && data.tags.length == 0 && data.visibility === "") {
        return;
      }
    } else {
...
function init_table_announcements(manual) {
  ...
  initDataTable(".table-announcements", admin_url + "announcements", undefined, undefined, "undefined",
    [1, "desc"]);
}
...
function init_table_tickets(manual) {
  ...
  if (typeof manual == "undefined" && ($("body").hasClass("dashboard") || $("body").hasClass("single-ticket"))) {
    return false;
  }
  ...
  _table_api = initDataTable(".tickets-table", _tickets_table_url, ticketsTableNotSortable, ticketsTableNotSortable, TicketServerParams,
...
function init_table_staff_projects(manual) {
  ...
  initDataTable(".table-staff-projects", admin_url + "projects/staff_projects", "undefined", "undefined", staffProjectsParams,
    [2, "asc"]);
}
...
function timer_action(e, task_id, timer_id, adminStop) {
  ...
  if (timer_id !== "" && task_id == "0") {
    ...
    popupData.content += "</div>";
    popupData.content += "<button type='button' onclick='timer_action(this,document.getElementById(\"timer_add_task_id\").value," + timer_id + ");return false;' class='btn btn-primary'>" + app.lang.confirm + "</button>";
...
function _init_timers_top_html(data) {
  ...
  data.total_timers > 0 ? $("#top-timers").find("i").addClass("tw-animate-spin") : $("#top-timers").find("i").removeClass("tw-animate-spin");
...
function tasks_bulk_action(event) {
  if (confirm_delete()) {
    ...
    if (mass_delete == false || typeof mass_delete == "undefined") {
      ...
      if (data.status === "" && data.priority === "" && data.tags === "" && data.assignees === "" && data.milestone === "" && data.billable === "") {
        return;
      }
    } else {
      data.mass_delete = true;
...
function exclude_tax_from_amount(tax_percent, total_amount) {
  totalTax = accounting.toFixed(
    (total_amount * tax_percent) / (100 + tax_percent), app.options.decimal_places);
...
function init_invoices_total(manual) {
  ...
  if ($("body").hasClass("invoices-total-manual") && typeof manual == "undefined" && !_inv_total_href_manual.hasClass("initialized")) {
    return;
  }
...
function init_estimates_total(manual) {
  ...
  if ($("body").hasClass("estimates-total-manual") && typeof manual == "undefined" && !_est_total_href_manual.hasClass("initialized")) {
    return;
  }
...
function proposal_pipeline_open(id) {
  ...
  requestGet("proposals/pipeline_open/" + id).done(function(response) {
    ...
    if (!visible) {
      ...
      $("#proposal").find(".modal.proposal-pipeline-modal").removeClass("fade").addClass("in").css("display", "block");
    }
  });
}
...
function estimate_pipeline_open(id) {
  ...
  requestGet("estimates/pipeline_open/" + id).done(function(response) {
    ...
    if (!visible) {
      ...
      $("#estimate").find(".modal.estimate-pipeline").removeClass("fade").addClass("in").css("display", "block");
    }
  });
}
    ...
    $.each(date_array, function(i, string) {
      if (string.length == 4) {
        ...
        if ($pYear.hasClass("format-n-yy")) {
          ...
          var month_index;
          if (app.options.date_format == "d-m-Y" || app.options.date_format == "d/m/Y" || app.options.date_format == "Y-m-d" || app.options.date_format == "d.m.Y") {
            month_index = 1;
...
function unformat_date(date) {
  ...
  if (app.options.date_format == "d-m-Y" || app.options.date_format == "d/m/Y" || app.options.date_format == "d.m.Y") {
    ...
    app.options.date_format == "m-d-Y" || app.options.date_format == "m.d.Y" || app.options.date_format == "m/d/Y") {
    day_index = 1;
    month_index = 0;
    year_index = 2;
  }
...
function set_notification_read_inline(id) {
  requestGet("misc/set_notification_read_inline/" + id).done(function() {
    ...
    notification.find(".notification-handler").removeClass("unread-notification");
...
editorConfig.setup = function(editor) {
  initializeTinyMceMentions(editor, function() {
    return $.getJSON(admin_url + "tasks/get_staff_names_for_mentions/" + taskid)
  })
};
...
function initializeTinyMceMentions(editor, usersCallback) {
  ...
  editor.perfexCommands.getUsersForMention = async function() {
    ...
    editor.perfexCommands.insertMentionUser = function(id, name, rng) {
      ...
      editor.insertContent(('<span class="mention" contenteditable="false" data-mention-id="' + id + '">@' + name + "</span>&nbsp;"))
    }
  editor.ui.registry.addAutocompleter('mentions', {
    ...
    fetch: function(pattern) {
      return new Promise(resolve => resolve(editor.perfexCommands.getUsersForMention()))
    },
